...
All relative paths in this skill are relative to the `docx` skill directory, not the project root.

In a project workspace, resolve paths like this:

- `scripts/office/unpack.py` means `skills/docx/scripts/office/unpack.py`
- `scripts/office/pack.py` means `skills/docx/scripts/office/pack.py`
- `scripts/office/validate.py` means `skills/docx/scripts/office/validate.py`
- `scripts/office/soffice.py` means `skills/docx/scripts/office/soffice.py`
- `scripts/accept_changes.py` means `skills/docx/scripts/accept_changes.py`

Never resolve these paths as `project_root/scripts/...`.
...
### Delivery Rule For Rewrite Tasks

If the task is to revise, humanize, de-flavor, polish, or otherwise edit an existing `.docx`,
the final deliverable must be a new `.docx` file unless the user explicitly asks for plain text only.

Treat extracted `.txt`, `.md`, or XML snippets as intermediate artifacts only.
Do not stop after text extraction or text rewriting.
Do not report success after producing only a Markdown or plain-text file.

The required completion path is:

1. unpack the source `.docx`
2. extract or inspect the editable text
3. rewrite the text
4. write the rewritten text back into the unpacked XML structure
5. repack with `pack.py`
...
**For rewrite workflows, this step is mandatory.**
After rewriting content, you must update the unpacked XML and run `pack.py` to produce the final `.docx`.
Do not substitute `output.md`, `output.txt`, or other extracted artifacts for the final document.
