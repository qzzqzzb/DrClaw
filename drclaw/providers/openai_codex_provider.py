"""OpenAI Codex Responses Provider — OAuth-based, ported from nanobot."""
...
# Codex status → DrClaw stop_reason.
_FINISH_REASON_MAP = {
    "completed": "end_turn",
...
class OpenAICodexProvider(LLMProvider):
    ...
    async def complete(
        ...
        body: dict[str, Any] = {
            "model": _strip_model_prefix(self._model),
            "store": False,
            "stream": True,
        ...
        try:
            try:
                content, tool_calls, stop_reason = await _request_codex(
...
def _convert_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Convert OpenAI-format messages to Codex input items.

    System messages are skipped — system prompt is passed via the `system` parameter.
    """
...
async def _consume_sse(
    ...
    async for event in _iter_sse(response):
        ...
        if event_type == "response.output_item.added":
            ...
            stop_reason = _map_finish_reason(status)
...
def _map_finish_reason(status: str | None) -> str:
    return _FINISH_REASON_MAP.get(status or "completed", "end_turn")
